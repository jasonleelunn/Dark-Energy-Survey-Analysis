...
y1 = [F1(:,2)];
x2 = [F2(:,1)];
y2 = [F2(:,2)]; %Obtaining the x and y values from each array of normally distrobuted points.
x3 = [F3(:,1)];
y3 = [F3(:,2)];
...
count = histogram2(X,Y,'BinWidth',[CellSize, CellSize],...
    'XBinLimits',[0 n],'YBinLimits',[0 n],'DisplayStyle','tile');
%title('Density of Objects in Cells of Set Angular Size Detected');
xlabel('x (Degrees)');
ylabel('y (Degrees)');
colorbar
